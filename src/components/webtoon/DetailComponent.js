...
import { getPalette } from "../../api/paletteApi";
          ...
          <div className="flex flex-col ml-10">
            ...
            <div className="flex">
              <Typography variant="body1" sx={{ mb: 1, fontWeight: 500 }}>
                {webtoon.rest == "N" ? convertDay(webtoon.updateDay) : <></>}
              </Typography>
              <Typography variant="body1" sx={{ mb: 1, fontWeight: 500 }}>
                {webtoon.rest == "Y" ? "휴재" : <></>}
              </Typography>
            ...
            <Typography variant="body1" sx={{ mb: 1, fontWeight: 500 }}>
              {convertGenre(webtoon.genre)}
            </Typography>
            <Typography variant="body1" sx={{ mb: 1, fontWeight: 500 }}>
              {webtoon.platform == "naver" ? "네이버웹툰" : "카카오웹툰"}
            </Typography>
            <div>
              {webtoon.fanCount != 0 ? (
                <VisibilityIcon
                  sx={{ fontSize: 15, color: grey[500], mr: 1 }}
                />
              ) : (
                <></>
              )}
              <span className="mr-3 text-zinc-500 text-sm">
                {convertFanCount(webtoon.fanCount)}
              </span>
              {webtoon.likeCount != null ? (
                <FavoriteIcon sx={{ fontSize: 15, color: grey[500], mr: 1 }} />
              ) : (
                <></>
              )}
            ...
            <div className="absolute bottom-1">
              <Button
                variant="contained"
                size="large"
                sx={{ width: 200, height: 60, boxShadow: "none" }}
                onClick={postUrl}
              >
